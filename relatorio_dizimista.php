<?php
	...
	$result_transacoes = "SELECT * FROM cliente WHERE dizimista ='sim'ORDER BY id ";
	...
	while($row_transacoes = mysqli_fetch_assoc($result)){
		$html .= '<tr><td>'.$row_transacoes['nome'].'';
			...
			$html .= '<td>'.$row_transacoes['dizimista'] . '<td>';
...
echo  $row;
	...
	// Carrega seu HTML
	$dompdf->load_html('
			<h1 style="text-align: center;">  Relatório Todos os Dizimistas</h1>
			'. $html .'
		');
